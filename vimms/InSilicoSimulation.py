...
def run_TopN(chems, scan_duration, params, out_dir):
    ...
    mass_spec = IndependentMassSpectrometer(
        params["ionisation_mode"], chems, scan_duration=scan_duration
    )
    env = Environment(
        mass_spec,
        controller,
...
def run_single_SmartROI(params):
    ...
    mass_spec = IndependentMassSpectrometer(
        params["ionisation_mode"], params["chems"], scan_duration=params["scan_duration"]
    )
    env = Environment(
        mass_spec,
        controller,
...
def extract_boxes(seed_file, out_dir, mzmine_command, xml_file):
    ...
    pick_peaks(
        [seed_file], xml_template=xml_file, output_dir=out_dir, mzmine_command=mzmine_command
    )
