...
class NonOverlapController(RoiController):
    ...
    class MS2Scheduler():
        ...
        def schedule_ms2s(self, new_tasks, ms2_tasks, mz, intensity):
            ...
            dda_scan_params = self.parent.get_ms2_scan_params(mz, intensity, precursor_scan_id, self.parent.isolation_width, self.parent.mz_tol, self.parent.rt_tol)
    ...
    def _process_scan(self, scan):
        ...
        if self.length_units == "scans": self.current_roi_length = np.array([roi.n for roi in self.live_roi])
        else: self.current_roi_length = np.array([roi.length_in_seconds for roi in self.live_roi])
    ...
    def _non_overlaps(self): 
        f = self.grid.get_estimator()
        return [self.grid.non_overlap(r.to_box(self.min_rt_width, self.min_mz_width, rt_shift=(-f(r)))) for r in self.live_roi]
    def _get_scores(self): return self._get_top_N_scores(self._get_dda_scores() * self._non_overlaps())
    def after_injection_cleanup(self): self.grid.update()