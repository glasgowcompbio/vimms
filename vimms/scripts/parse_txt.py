...
class SequenceItem:
    ...
    def __str__(self):
        return "{}, {}: {}, {}".format(
            self.running_number, self.ms_level, self.duration.total_seconds(), 60 * self.start_time
        )
...
def main():
    ...
    if args.plot_from is not None or args.plot_to is not None:
        ...
        plot_list = filter(lambda x: plot_from <= x.running_number <= plot_to, scan_sequence_list)
