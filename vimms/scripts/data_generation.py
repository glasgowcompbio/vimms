...
if __name__ == "__main__":
    ...
    parser.add_argument("--min_mz", dest="min_mz", default=DEFAULT_MS1_SCAN_WINDOW[0], type=float)
    parser.add_argument("--max_mz", dest="max_mz", default=DEFAULT_MS1_SCAN_WINDOW[1], type=float)
    parser.add_argument("--min_rt", dest="min_rt", default=DEFAULT_RT_RANGE[0], type=float)
    parser.add_argument("--max_rt", dest="max_rt", default=DEFAULT_RT_RANGE[1], type=float)
    parser.add_argument(
        "--min_ms1_sampling_intensity", dest="min_ms1_sampling_intensity", default=1e3, type=float
    )
    parser.add_argument(
        "--max_ms1_sampling_intensity", dest="max_ms1_sampling_intensity", default=1e9, type=float
    )
    parser.add_argument("--formula_database_file", dest="formula_database_file", type=str)
    ...
    parser.add_argument("--output_msp_file", dest="output_msp_file", default=None, type=str)
    parser.add_argument("--spike_max", dest="spike_max", default=1000, type=float)
    ...
    cm = ChemicalMixtureCreator(
        fs, rt_and_intensity_sampler=ri, ms2_sampler=cs, adduct_prior_dict=ADDUCT_DICT_POS_MH
    )
    ...
    if args.output_swath_file is not None:
        ...
        ms = IndependentMassSpectrometer(
            POSITIVE_IONISATION_MODE, dataset, spike_noise=spike_noise
        )
