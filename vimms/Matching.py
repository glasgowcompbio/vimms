...
#TODO: fullscan info should be usable to more precisely give intensity at particular MS2 time than using precursor
#TODO: bin sort version
#TODO: constraint programming/stable marriage versions??
#TODO: general weighted matching is slow, need to find weighted bipartite algorithm in another package
#TODO: could chemical collapse with arbitrary exclusion condition, for when chems_list doesn't have same items e.g. with aligner
...
class MatchingChem():
    ...
    def mzmine2nodes(box_file_path):
        with open(box_file_path, "r") as f:
            ...
            min_mz = [i for i, fd in enumerate(fields) if fd.strip().endswith("Peak m/z min")]
            max_mz = [i for i, fd in enumerate(fields) if fd.strip().endswith("Peak m/z max")]
            min_rt = [i for i, fd in enumerate(fields) if fd.strip().endswith("Peak RT start")]
            max_rt = [i for i, fd in enumerate(fields) if fd.strip().endswith("Peak RT end")]
            
            if(len(min_mz) == 0): print("No minimum m/z could be found for mzmine box file!") 
            if(len(max_mz) == 0): print("No maximum m/z could be found for mzmine box file!")
            if(len(min_rt) == 0): print("No minimum peak rt could be found for mzmine box file!")
            if(len(max_rt) == 0): print("No maximum peak rt could be found for mzmine box file!")
...
class Matching():
    ...
    def make_edges(scans, chems, intensity_threshold):
        ...
        for ch in chems: rt_intervals.addi(ch.min_rt, ch.max_rt, ch)
        ...
        for s in scans:
            if(s.ms_level == 1):
                ...
                for ch in intersected: 
                    ...
                    seen_intensities[ch] = max(ch.intensity, seen_intensities.get(ch, 0))
                ...
                for ch in active_chems: 
                    ...
                    seen_edges.add(ch)
        for ch in chems: ch.intensity = None #clear temporary value
        
        print(f"|chems| BEFORE COLLAPSE: {len(chems)}")
        print(f"num chems without edges: {sum(1 for _, ls in edges.items() if ls == [])}")
        print(f"chems without edges: {[ch for ch, ls in edges.items() if ls == []][:10]}")
        ...
        #print(f"intensity counts: {Counter(v for _, v in seen_intensities.items())}")
    ...
    def build_nx_graph(scans, chems, edges):
        G = nx.Graph()
        for s in scans:
            if(s.ms_level > 1): G.add_node(s, bipartite=0)
        for ch in chems: G.add_node(ch, bipartite=1)
