...
class SequenceItem(object):
    ...
    def __str__(self):
        return "{}, {}: {}, {}".format(self.running_number,self.ms_level,self.duration.total_seconds(),60*self.start_time)
    ...
    for pos,line in enumerate(lines):
        if 'Placing' in line:
            ...
            if running_number < start_no:
                continue
            if not running_number in scan_sequence_dict:
                scan_sequence_dict[running_number] = SequenceItem(running_number)
            scan_sequence_dict[running_number].add_send(get_time(line))
            scan_sequence_dict[running_number].ms_level = get_next_ms_level(lines,pos)
            scan_sequence_dict[running_number].fusion_accept = get_previous_can_accept(lines,pos)
        if 'Received' in line:
            running_number = get_running_number(line)
            if running_number < start_no:
                continue
            ...
            scan_sequence_dict[running_number].start_time = get_next_start_time(lines,pos)
            scan_sequence_dict[running_number].inject_time = get_next_inject_time(lines,pos)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--plot_from',dest='plot_from',type=int,default=None)
    ...
    if args.plot_from is not None or args.plot_to is not None:
        ...
        plot_list = list(filter(lambda x: x.running_number >= plot_from and x.running_number <= plot_to, scan_sequence_list))
        for s in plot_list:
            print(s)
        ...
        for i,s in enumerate(plot_list):
            ...
            text_string = "{} (i{:.0f})".format(str(s.duration.total_seconds()),s.inject_time)
