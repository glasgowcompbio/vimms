...
class ThresholdEstimator(NoiseEstimator):
    ...
    def estimate_noise(self, roi): 
        ...
        return float(max(itertools.accumulate(aboves, lambda a, b: a + 1 if b else 0, initial=0)) >= self.count_threshold)

#It's a peak if we observe intensity increasing (maybe a rolling average) for N scans in a row
class IncreaseEstimator(NoiseEstimator):
    def __init__(self, count_threshold):
        self.count_threshold = count_threshold
        
    def estimate_noise(self, roi):
        increases = (roi.intensity_list[i+1] - roi.intensity_list[i] > 0 for i in range(len(roi.intensity_list) - 1))
        return float(max(itertools.accumulate(increases, lambda a, b: a + 1 if b else 0, initial=0)) >= self.count_threshold)
...
class NoiseController(RoiController):
    def __init__(self, ionisation_mode, isolation_width, mz_tol, min_ms1_intensity, min_roi_intensity,
                 min_roi_length, N, noise_estimator, rt_tol=10, min_roi_length_for_fragmentation=1, length_units="scans", ms1_shift=0,
                 params=None):
        super().__init__(
        ...
        self.noise_estimator = noise_estimator #given a RoI, returns probability that it is noise
