...
class ColourMap():
    ...
    def add_to_subplot(self, ax, boxes, key):
        for box, colour in self.assign_colours(boxes, key):
            ax.add_patch(
                patches.Rectangle((box.pt1.x, box.pt1.y), box.pt2.x - box.pt1.x, box.pt2.y - box.pt1.y, linewidth=1,
...
class AutoColourMap(ColourMap):
    ...
    def assign_colours(self, boxes):
        ...
        def interpolate_lower(box):
            return top_level_colours[box.parents[0]].interpolate([top_level_colours[b] for b in box.parents[1:]])
...
class PlotPoints():
    ...
    def get_points_in_bounds(self, min_rt=None, max_rt=None, min_mz=None, max_mz=None):
        active_ms1 = self.bound_points(self.ms1_points, min_rt=min_rt, max_rt=max_rt, min_mz=min_mz,
                                       max_mz=max_mz) if len(self.ms1_points) > 0 else np.array([[]])
        active_ms2 = self.bound_points(self.ms2s, min_rt=min_rt, max_rt=max_rt, min_mz=min_mz, max_mz=max_mz) if len(
    ...
    def points_in_bounds(self, min_rt=None, max_rt=None, min_mz=None, max_mz=None):
        self.active_ms1, self.active_ms2 = self.get_points_in_bounds(min_rt=min_rt, max_rt=max_rt, min_mz=min_mz,
                                                                     max_mz=max_mz)
    ...
    def plot_ms1s(self, ax, min_rt=None, max_rt=None, min_mz=None, max_mz=None, abs_scaling=False):
        ...
        rts, mzs, intensities = self.ms1_points[self.active_ms1, 0], self.ms1_points[self.active_ms1, 1], \
    ...
    def plot_ms2s(self, ax, min_rt=None, max_rt=None, min_mz=None, max_mz=None, abs_scaling=False):
        self.points_in_bounds(min_rt=min_rt, max_rt=max_rt, min_mz=min_mz, max_mz=max_mz)
        rts, mzs, intensities = self.ms2s[self.active_ms2, 0], self.ms2s[self.active_ms2, 1], self.ms2s[
            self.active_ms2, 2]
...
class PlotBox():
    ...
    def __repr__(self):
        return f"PlotBox(min_rt={self.min_rt}, max_rt={self.max_rt}, min_mz={self.min_mz}, max_mz={self.max_mz}, apex_intensity={self.intensity})"
    ...
    def plot_box(self, ax, mzml, abs_scaling=False, other_boxes=[], rt_buffer=None, mz_buffer=None):
        ...
        pts.plot_ms1s(ax, min_rt=xbounds[0], max_rt=xbounds[1], min_mz=ybounds[0], max_mz=ybounds[1],
