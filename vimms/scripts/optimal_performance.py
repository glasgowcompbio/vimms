...
from vimms.mass_spec_utils.data_import.mzmine import load_picked_boxes
...
def add_rois_to_boxes(boxes, mzfile, verbose=True):
    ...
    for i, scan in enumerate(mzfile.scans):
        ...
        ok_boxes = list(
            filter(
                lambda x: x.rt_range_in_seconds[0] <= scan_rt
                and x.rt_range_in_seconds[1] >= scan_rt,
                boxes,
            )
        )
        if len(ok_boxes) == 0:
...
def make_edges_chems(chems, scan_start_times, scan_levels, min_ms1_intensity, chrom_min=0.001):
    ...
    for chemical in chems:
        ...
        max_intensity = (
            chemical.isotopes[which_isotope][1] * adduct_intensity[adduct] * chemical.max_intensity
        )
