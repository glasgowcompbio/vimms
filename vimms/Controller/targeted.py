...
class Target:
    ...
    def peak_in(self, mz, rt):
        if (self.from_mz <= mz <= self.to_mz) and (self.from_rt <= rt <= self.to_rt):
            return True
        else:
            return False
    ...
    def active(self, mz_intensity, rt, min_intensity_for_fragmentation):
        ...
        sub_mzi = list(
            filter(
                lambda x: x[0] >= self.from_mz
                and x[0] <= self.to_mz
                and x[1] >= min_intensity_for_fragmentation,
                mz_intensity,
            )
        )
        if len(sub_mzi) > 0:
    ...
    def __str__(self):
        if self.name is not None:
            return "{}{} (m/z: {}->{}, rt: {}->{})".format(
                self.name, self.adduct, self.from_mz, self.to_mz, self.from_rt, self.to_rt
            )
        else:
            return "(m/z: {}->{}, rt: {}->{})".format(
