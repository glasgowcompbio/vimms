...
class LineSweeper:
    ...
    def _group_endpoints(endpts):
        return (list(v) for k, v in itertools.groupby(endpts, key=attrgetter("pos")))
            ...
            for begin, end in invs:
                ...
                for y_group in self._group_endpoints(y_ends):
                    ...
                    if y_active != [] and y_pos > begin:
                        prev_intervals.addi(
                            max(begin, prev_y_pos), min(end, y_pos), (x_pos, copy.copy(y_active))
                        )

                    for y_pos, y_end, y_box in y_group:
                        if y_end:
                            y_active.remove(y_box)
                        else:
                            y_active.append(y_box)
...
class BoxGrid(BoxExact):
    def __init__(
        self, min_rt=0, max_rt=1440, rt_box_size=50, min_mz=0, max_mz=2000, mz_box_size=1
    ):
    ...
    def point_in_box(self, pt):
        ...
        return any(b.contains_point(pt) for b in self.grid.get_boxes(box))

    def point_in_which_boxes(self, pt):
        box = GenericBox(pt.x, pt.x, pt.y, pt.y)
        return set(b for b in self.grid.get_boxes(box) if b.contains_point(pt))

    def interval_overlaps_which_boxes(self, inv):
        box = GenericBox(inv.pt1.x, inv.pt2.x, inv.pt1.y, inv.pt2.y)
        return set(b for b in self.grid.get_boxes(box) if b.overlaps_with_box(box))

    def interval_covers_which_boxes(self, inv):
        box = GenericBox(inv.pt1.x, inv.pt2.x, inv.pt1.y, inv.pt2.y)
        return set(b for b in self.grid.get_boxes(box) if b.interval_contains(inv))

    def _get_overlapping_boxes(self, box):
        return set(b for b in self.grid.get_boxes(box) if box.overlaps_with_box(b))
    ...
    def register_boxes(self, boxes):
        ...
        resize = any(
            min_rt > b.pt1.x or max_rt < b.pt2.x or min_mz > b.pt1.y or max_mz < b.pt2.y
            for b in boxes
        )
...
class BoxIntervalTrees(BoxExact):
    ...
    def get_all_boxes(self):
        return (inv.data for inv in self.y_tree.items())

    def point_in_box(self, pt):
        return any(yb.data.pt1.x <= pt.x and yb.data.pt2.x >= pt.x for yb in self.y_tree.at(pt.y))
