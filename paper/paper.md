...
during data acquisition directly affects the quality and coverage of identification results. Developing novel
fragmentation strategies is challenging due to the high experimental cost of running an actual mass spectrometry
instrument and the lack of programmable simulation environment that could be used to support developmental activities.
ViMMS 2.0 is a framework that could be used to develop new fragmentation strategies completely *in-silico* in
metabolomics. The framework allows users to generate chemical objects (produced synthetically or extracted from existing
mzML files)
and run them through a simulated tandem mass spectrometry process, where different fragmentation strategies could be
implemented, tested and evaluated. In this paper, we highlight the software design choices of the framework and
illustrate with examples how a fragmentation strategy could be implemented in ViMMS 2.0.
...
changes. Liquid chromatography (LC) coupled to mass spectrometry (MS) is commonly used to identify small molecules in
...
spectral databases. An important factor that determines how many molecules could be matched to databases using
...
Existing mass spectrometry simulators are ill-fitted to support the development of fragmentation strategies that respond
to incoming scans in real time. This is primarily due to the restricted ways users can add a new strategies in these
simulators' framework or software architecture. Currently available simulators such as as
Mspire-Simulator [@noyce2013mspire], JAMSS [@smith2015jamss], OpenMS-Simulator [@wang2015openms],
MSAcquisitionSimulator [@goldfarb2016msacquisitionsimulator] and SMITER [@kosters2021smiter] operate on proteomics data,
exist as stand-alone programs or GUI applications, and are not easily scriptable or programmable.

Our work in ViMMS 1.0 [@Wandy2019-ok] was the first simulator that allowed for a metabolomics-based simulation
environment that could easily extended in Python. However ViMMS 1.0 suffered from several weaknesses: its codebase was
monolithic, making it difficult to instantiate input from different sources or to introduce different extensions to the
base functionalities, including adding new fragmentation strategies. The focus of ViMMS 1.0 was more on simulating a
complete tandem mass spectrometry run in metabolomics, and not as much as in enabling the development of new strategies.

# Improved ViMMS 2.0 Framework

In ViMMS 2.0 we significantly improved the simulator framework architecture with the aim of reducing coupling between
modules. This improved modularity allows many aspects of the framework to be swapped out with alternative
implementations, including classes implementations of new fragmentation strategies. The simulator framework consists of
two core functionalities: the generation of chemicals from multiple sources, and executions of fragmentation strategies
implemented as controller classes. An overview of the overall architecture is given in \autoref{diagram}.
...
intensities, chromatographic shapes and generated MS2 spectra, as well as scan timing.
