from vimms.InSilicoSimulation import (
    ...
    run_WeightedDEW,
    extract_boxes,
    evaluate_boxes_as_dict,
    evaluate_boxes_as_array,
    save_counts,
    string_to_list,
    plot_counts,
)
...
class InSilicoSimulator(ABC):
    ...
    def get_scan_duration(self):
        ...
        time_dict = (
            get_timing(time_dict_str) if len(time_dict_str) > 0 else extract_timing(self.seed_file)
        )
        return time_dict
...
class SmartROISimulator(InSilicoSimulator):
    def get_params(self):
        ...
        min_frag = self.config_parser.getint("simulation", "min_roi_length_for_fragmentation")
    ...
    def evaluate(self, params):
        ...
        out_file = os.path.join(
            self.out_dir, "%s_%s.png" % (self.controller_name, self.sample_name)
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="In-silico Optimisation of Fragmentation " "Strategy using ViMMS"
    )
    ...
    choices = {
        IN_SILICO_OPTIMISE_TOPN: TopNSimulator(
            sample_name, seed_file, out_dir, controller_name, config_parser
        ),
        IN_SILICO_OPTIMISE_SMART_ROI: SmartROISimulator(
            sample_name, seed_file, out_dir, controller_name, config_parser
        ),
        IN_SILICO_OPTIMISE_WEIGHTED_DEW: WeightedDEWSimulator(
            sample_name, seed_file, out_dir, controller_name, config_parser
        ),
    }
